...
export function parsePromptTemplateMarkdown(
  ...
  if (!idMatch) throw new Error(`Prompt 模板缺少 id 头: ${source}`);
  ...
  return validatePromptTemplateFile(
    { id: idMatch[1].trim(), system: sections.get('system'), user: sections.get('user') },
    source,
  );
}

function validatePromptTemplateFile(input: unknown, source: string): PromptTemplateFile {
  if (!input || typeof input !== 'object') throw new Error(`Prompt 模板格式非法: ${source}`);
  ...
  if (!id) throw new Error(`Prompt 模板缺少有效 id: ${source}`);
  if (system === undefined && user === undefined) throw new Error(`Prompt 模板至少需要 system 或 user 之一: ${source}`);
...
for (const [source, raw] of Object.entries(bundledPromptSources)) {
  ...
  if (promptTemplateMap.has(template.id)) throw new Error(`Prompt 模板 id 重复: ${template.id} (${source})`);
...
export function loadPromptTemplateFile(id: string): PromptTemplateFile {
  ...
  if (!template) throw new Error(`Prompt 模板不存在: ${id}`);
  return template;
}

export function renderPromptTemplate(template: PromptTemplateFile, variables: TemplateVariableMap = {}): RenderedPrompt {
  return {
...
export function renderPromptSectionTemplate(template: PromptTemplateFile, section: PromptSectionKey, variables: TemplateVariableMap = {}): string {
  const content = template[section];
  return content ? renderTemplate(content, variables) : '';
}

export function renderPrompt(id: string, variables: TemplateVariableMap = {}): RenderedPrompt {
  return renderPromptTemplate(loadPromptTemplateFile(id), variables);
}

export function renderPromptSection(id: string, section: PromptSectionKey, variables: TemplateVariableMap = {}): string {
  return renderPromptSectionTemplate(loadPromptTemplateFile(id), section, variables);
}

export function renderPromptSystem(id: string, variables: TemplateVariableMap = {}): string {
  return renderPromptSection(id, 'system', variables);
}

export function renderPromptUser(id: string, variables: TemplateVariableMap = {}): string {
